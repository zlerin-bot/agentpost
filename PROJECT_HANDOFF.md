...
- 2026-09-14 **紧凑消息卡，本地完成、未部署**：作者/时间→主题→正文→共享范围/接收状态/回复；移除消息内黄色信息框、蓝色正文框及“说了什么”标题。短文完整显示，长文五行预览/展开全文，保留安全文本及Markdown段落列表；不重复摘要。Agent简短署名可展开完整名称；相同回复主题不重复显示。JS52通过、语法/diff通过；8788桌面及390px单框长文、收起/键盘展开、console无错误通过。包含上一切片分区/倒序，生产仍0.1.71；定时任务暂停。

- 2026-09-14 **工作分区与讨论倒序，本地完成未部署**：将历史区拆为“长期未推进”（7天无状态更新且非活跃执行）和“已结束”；AI运行详情移除未结束数字；排队提醒注明原工作时间及补交/改派动作，不自动关闭旧派工。讨论组内默认倒序，支持从头阅读，移动既有卡片保留正文展开/草稿。JS52通过、语法/diff通过；8788合成任务桌面/390px正倒序、键盘及console检查通过，手机排序条布局已修正。生产仍0.1.71，定时任务暂停。

- 2026-09-14 **0.1.71 / aff1e44 已部署，deployed_https_verified**：默认开放陌生联系（保留显式关闭）、姓名发现/好友入口、正文摘录/引用字号均上线。单包一次上传；stage/deploy/postflight=ok，切换51秒/后检2秒；schema0045不变。公网mars已解析为mars lee（@mars），登录态开关开启、任务正文摘录及展开无重复实测通过。非PG569通过、2跳过、7排除；Nginx/PG原PID不变。备份及SHA详见 docs/DEPLOYMENT_0.1.71_20260914.md。定时任务暂停；未推送GitHub/未另发测试通知，跨设备及真实宿主验收仍待确认。下文未部署为历史状态。

- 2026-09-14 **0.1.71准备发布**：用户明确要求陌生联系默认开放并部署近几次调整。无偏好记录的新老账号默认开放，已有false记录保持关闭；通过外连接按读取时默认值实现，无批量数据改写、无schema变更（0045）。收到不等于执行。包含姓名候选确认、好友发现、收起正文摘录/小字号来源。完整非PG首轮563通过/6个旧版本断言失败；已同步版本断言，相关50项通过，TS8通过，Ruff/format/lock通过。发布后补充证据。定时任务暂停。

- 2026-09-14 **讨论摘录与引用字号，本地完成、未部署**：“说了什么”收起时显示正文开头最多180字符/三行，展开隐藏摘录避免重复；回复/引用来源改为caption字号与灰色，保持可点击。JS52项、语法、diff检查通过；8788合成长正文390px实看、展开隐藏摘录、正文可见、console无错误。生产仍0.1.70；陌生联系默认开放尚未实施，定时任务保持暂停。
...
- 2026-09-14 **讨论元信息修正 local component verified，未部署**：移除普通讨论及讨论组摘要中无生产者的“谁跟进/跟进”，删除死字段读取和样式。主题消息显示“讨论主题”，明确回复显示“回应对象”，无主题不显示占位；“面向谁/面向”改为“共享范围”，不冒充被要求回复者或负责人。已安排工作的真实负责人/状态继续由原 Assignment 界面展示；当前 source_activity_id 指向 assignment_created，没有讨论到派工的直接关系，本轮没有猜测关联或新增派工功能。
- 验证：JS syntax、50项导航测试、git diff --check通过；使用实际元信息函数和CSS的隔离合成组件页面验证桌面1280及390px，手机scrollWidth=clientWidth=390，键盘正文展开/收起通过，console error/warn=0。没有完整任务页、生产或真机验收；不涉及Python/schema修改。本轮仅处理最近讨论的元信息问题，访客接入反馈完善和部署仍待后续，定时任务保持暂停。
- 9月14日已读取的新评审：020希望将“申请好友并发起协作”与接收方“接受并开始协作”合并步骤；alalei建议普通问候只允许一次回复，好友及任务不自动创建。建议按明确协作请求与普通问候分流；还包括二维码/同一公开链接、避免重复输入与注册接续、准确提示和移动验收。尚未实现这些评审建议，不应将下文原始访客切片直接视为最终上线候选。

- 2026-09-12 **访客首次联系已发测试任务评审，等待新反馈后完善并部署**：用户已明确授权“先把思路/已做开发发给测试任务，再根据反馈完善并部署”。已复用mars agent向已解析验证的测试任务 `bd3c05dc-d67c-44d1-a4fc-0692a1d415f6` 发布评审说明，activity `b12e510a-b393-4b76-af7b-f6e1f7775ef6`，publication_origin=human_delegated，附件0、Run0。点名020/张子良/dylan/alalei讨论首次使用步骤、注册接续、身份/状态权限、手机操作、默认关闭和接受门槛；明确本地272624b未上线，不能在生产复测新入口。发送后读取时活动总数257，尚无本轮新回复。不要把早前0.1.69复测当成本轮评审，不要提前部署。收到实质反馈后继续完成修复、验证、发行版本及安装包一致性、0044生产迁移演练和受保护发布；无需为已授权部署再次询问。既有定时任务保持暂停，没有新建轮询自动任务。

- 2026-09-12 **访客首次联系首个可用切片 local_verified，未部署**：公开用户名主动开启后允许精确查找；未注册AI可从llms.txt/公开合同发现接口并使用单次访客凭证投递文字、查回执、返回注册接续链接。新增/contact网页投递/接收设置/接受拒绝/认领/接续、侧栏及移动设置待处理入口；默认AI可经SDK/MCP只读分页查看请求，握手含入口。双方接受与注册认领及有效默认Agent齐备后原子转成Task并保留原始内容；未接受不创建Run、不自动好友、不创建重复Agent。短期HttpOnly凭证恢复浏览器刷新；访客不能读任务，内容安全文本渲染，持久限流/幂等/失权404/CSRF/7天有效期生效。
- 验证：完整非PG559 passed、2 skipped、7 deselected；随后分页补充后的最终专项17 passed（其中6项访客集成测试），JS50项、两脚本语法、Ruff/format/diff通过。实际桌面及390px无横向溢出，查找投递/刷新恢复/登录接受/键盘拒绝实测，无脚本错误。SQLite并发接续只建一个Task、两个参与Run；0043等价模型基线→0044→0043→0044往返通过。完整历史SQLite从零迁移仍有旧handle重复列问题，本轮未修。真实PostgreSQL、跨设备注册和各宿主端到端未验收。
- 新schema为0044_guest_contacts；未升级公开发行版本/安装包、未部署、未推送或发送生产通知，生产仍0.1.69/7d570c3。模糊候选/附件/AI自动接受/陌生请求主动唤醒和邮件提醒/自动安装宿主未实现；首次未连接AI的用户仍需既有授权连接流程。详细设计/边界见 docs/GUEST_FIRST_CONTACT_20260912.md。合成预览 http://127.0.0.1:8786/contact ，启动脚本/private/tmp/ap_contact_resume.py；保留现有演示库，勿重新seed。定时任务仍暂停。

- 2026-09-12 **正文重复摘要修正 local_verified，未部署**：删除“说了什么”卡片中的正文摘录，展开直接显示完整正文；提示随展开状态显示“展开阅读/收起正文”。外层 summary 样式限定为直接子元素，修复“查看原始正文”入口被网格挤成竖排。JS 语法、50 项导航测试、diff check 通过；使用实际组件函数和 CSS 的隔离合成演示验证桌面及390px、键盘收起、原文展开，无横向溢出及脚本错误。8783原预览服务已停止，本轮未重建其数据库；未做完整任务页或真机验收。测试任务最新反馈为020于9月12日15:43发布的 `cf6dee2d-1f63-4d4f-aa42-10e38f66bdfc`：最新协作、派工分层、文件目录及DOCX正文复测通过；PDF仅明确预览窗口打开，逐页渲染未明确验证；手机及ZIP仍未测。前次新讨论快捷跳转、任务简报入口、旧连接升级仍待专项验收。定时任务保持暂停。
- GitHub状态更正：用户授权后仓库已改名 `zlerin-bot/agentpost`；品牌提交 `52a16bc8ab9aefdfae19d41ffc6d44bc23564fad` 已推送 main 与 codex/task-center-v0.1.34，并核对远端一致。下文GitHub阻塞为历史状态。生产仍0.1.69/7d570c3，品牌提交未部署。
