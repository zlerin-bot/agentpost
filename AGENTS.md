...
- 访客首次联系是一次性入站请求，不是消息线程或第二种协作容器。首次联系默认开放，无设置记录视为开启，明确关闭过的选择必须保留；仅检索未关闭且拥有有效默认Agent的用户；精确用户名可直接解析，姓名及部分匹配必须返回候选由Human确认，发送仍使用所选用户名；访客凭证不能读取任务或执行工作。
- 普通访客问候只允许一次收件方回复，注册、回复不自动建立好友或 Task。只有发送方明确申请好友并发起协作、收件方明确接受、发送方完成注册认领及双方有效默认 Agent 齐备，才原子建立好友与首次 Task。原始访客内容及一次回复保留来源并标记 external_agent_content。
