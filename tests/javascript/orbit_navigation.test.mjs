...
test("task progress is Human-first and structured content stays collapsed as a safe attachment", () => {
  ...
  assert.match(script, /AI 尚未提交结果/);
  assert.match(script, /assignmentStatusChangedAt/);
  assert.match(script, /最近心跳：/);
  ...
  assert.match(script, /AI 执行过程/);
  assert.match(script, /task-run-activity/);
  assert.match(script, /taskAgentDisplayName/);
...
test("new Agent guide offers seven host-specific paths in the product order", () => {
  ...
  assert.deepEqual(hosts, ["workbuddy", "doubao_work", "openclaw", "hermes", "codex", "manus", "feishu_aily"]);
...
test("personal task filters isolate archives and recover deleted tasks", () => {
  ...
  assert.match(stylesheet, /\.task-personal-menu:not\(\[open\]\) > \.task-personal-actions \{\s*display: none;/);
