...
const replyMessageTypes = [...messageTypes, "result"];
const priorities = ["low", "normal", "high", "urgent"];
const formats = ["text", "markdown", "json"];
const inboxStatuses = ["unread", "delivered", "read", "acked"];
...
const attachmentIds = Type.Optional(Type.Array(Type.String({ format: "uuid" }), { maxItems: 32, uniqueItems: true }));
...
const configSchema = Type.Object({
    ...
    timeoutMs: Type.Optional(Type.Integer({ minimum: 100, maximum: 120_000, default: 30_000 })),
}, strict);
function api(config) {
    ...
    tools: (tool) => [
        tool({
            ...
            parameters: Type.Object({
                ...
                publication_origin: Type.Optional(Type.Union([
        ...
        tool({
            ...
            parameters: Type.Object({ query: Type.String({ minLength: 1, maxLength: 500 }) }, strict),
            outputSchema: resultSchema,
            async execute(params, config, context) {
                try {
        ...
        tool({
            ...
            parameters: Type.Object({ message_id: Type.String({ minLength: 1, maxLength: 64 }) }, strict),
            outputSchema: resultSchema,
            async execute(params, config, context) {
                try {
                    return result(await api(config).request({
