...
## 2026-09-07 P0 状态真实性补充

- Connector 连接心跳与任务监听心跳已经拆分。Human 页面以“可接任务、正在工作、恢复中、需要处理”表达行动含义，连接健康、监听状态、最近监听时间和唤醒能力只在详情中提供诊断。
- “可接任务”要求当前连接和任务监听心跳都新鲜；“正在工作”要求服务端存在 leased/starting/running 的 Agent Run。只有连接心跳而没有 Worker 的 Agent 会显示“需要处理”，排队工作也明确提示尚未被任务监听领取。
- `agentpost-task-worker` 会使用稳定的监听会话 ID 上报 listening/stopped/error。现有 Codex worker 仍需 Human 主动启动，`wake_capability=manual`；自动系统唤醒没有实现，不得把连接在线当作可自动唤醒。
- 新迁移 `0040_connector_task_listener_truth` 增加监听事实和约束；普通 Connector 心跳兼容旧客户端，且不会擦除最后一次监听事实。
- 其他原生宿主需要升级适配器并实际运行任务监听后才会进入“可接任务”。这是有意的保守状态，不以授权、安装成功或普通心跳推断执行能力。
